...
function resolvePreloadPath() {
    // When built by tsup, __dirname = dist/main
    const distCandidate = path.join(__dirname, '../preload/preload.js');
    if (fs.existsSync(distCandidate)) return distCandidate;

    // Fallback for odd dev layouts
    const alt1 = path.join(process.cwd(), 'dist/preload/preload.js');
    if (fs.existsSync(alt1)) return alt1;

    // As a last resort (very early dev), point at TS directly (Electron can load TS via ts-node only if configured; we just log)
    const tsCandidate = path.join(process.cwd(), 'main/preload.ts');
    console.warn('[preload] dist preload not found, candidate TS path:', tsCandidate);
    return distCandidate; // still return the expected dist path; existence is checked above
}

function log(...args: any[]) { console.log('[main]', ...args); }
function logErr(...args: any[]) { console.error('[main:err]', ...args); }
...
async function createWindow() {
    ...
    log('Resolved preload:', preloadPath, 'exists:', fs.existsSync(preloadPath));
...
ipcMain.handle('image:copy', async () => { return; });
